...
namespace Szyfr_Cezara
{
    public partial class Form1 : Form
    {
        ...
        private void Load_code_clik_Click(object sender, EventArgs e)
        {
            MessageBox.Show("Wprowadź ścieżkę tekstu jawnego");
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                text_to_codeee.LoadFile(openFileDialog1.FileName, RichTextBoxStreamType.PlainText);
            }
            MessageBox.Show("Wprowadź ścieżkę klucza");
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                codeee.LoadFile(openFileDialog1.FileName, RichTextBoxStreamType.PlainText);
            }
            text_to_code.Text = text_to_codeee.Text;
            code.Text = codeee.Text;
        }
        ...
        {
            using (FileStream filee = File.Create("szyfr.txt"))
            using (StreamWriter writer = new StreamWriter(filee))
                writer.WriteLine(text_code.Text);
            MessageBox.Show(@"Plik z szyfrem zapisano: Szyfr_Cezara\bin\Debug\szyfr.txt");
        }
        public void Savekey()
        {
            using (FileStream filee = File.Create("klucz.txt"))
            using (StreamWriter writer = new StreamWriter(filee))
                writer.WriteLine(code.Text);
            MessageBox.Show(@"Plik z kluczem zapisano: Szyfr_Cezara\bin\Debug\klucz.txt");
        }
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            text_to_decode.Text = text_to_codeee.Text;
            decode.Text = codeee.Text;
        }
        ...
        private void save_CheckedChanged_1(object sender, EventArgs e)
        {
        ...
        private void text_to_code_TextChanged_1(object sender, EventArgs e)
        {
